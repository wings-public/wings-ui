...
{
    public partial class uc_AddPoweruserForPI : System.Web.UI.UserControl
    {
        ...
        protected void btn_Insert_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                foreach (RadListBoxItem item in RLB_User.Items)
                {
                    ...
                    TGList.Add(new FSObject(Request.QueryString["PIID"].ToString(), PowerUserID));
                }
        ...
        public class ThreadAddFs
        {
            ...
            public void ThreadProc()
            {
                ...
                TG_table.Columns.Add("PowerUserID", typeof(Int32));
                
               

                for (int i = 0; i <= T_TGList.Count - 1; i++)
                {
                    TG_table.Rows.Add( T_TGList[i].PIID ,T_TGList[i].PowerUserID);
                }
                using (SqlConnection Conn = new SqlConnection(CnnStr))
                {
                    ...
                    {
                        s.DestinationTableName = "Tbl_PI_PowerUser";
