...
{
    public partial class uc_AddNewIndividualToIndividualset : System.Web.UI.UserControl
    {
        #region "CnnStr"{
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
            if (RLB_Individual.Items.Count.Equals(0))
            {
                RL_feedback.Text = "Please select a Individual";
            }
            else
            {
                List<FSObject> TGList = new List<FSObject>();

                foreach (RadListBoxItem item in RLB_Individual.Items)
                {
                    string IndividualID = item.Value.ToString();
                    TGList.Add(new FSObject(Request.QueryString["IndividualsetID"].ToString(), IndividualID, Session["WiNGSMasterId"].ToString()));
                }
        ...
        public struct FSObject
        {
            ...
            public FSObject(string IndividualsetID, string IndividualID, string CreatorID)
            {
        ...
        public class ThreadAddFs
        {
            ...
            public ThreadAddFs(List<FSObject> TGList)
            {
                ...
                {
                    T_TGList.Add(new FSObject(TGList[i].IndividualsetID, TGList[i].IndividualID, TGList[i].CreatorID));
                }
            }
            ...
            public void ThreadProc()
            {
                ...
                {
                    TG_table.Rows.Add(T_TGList[i].IndividualsetID, T_TGList[i].IndividualID,T_TGList[i].CreatorID);
                }
                using (SqlConnection Conn = new SqlConnection(CnnStr))
                {
                    ...
                    {
                        s.DestinationTableName = "Tbl_Individualset_Individuals";
