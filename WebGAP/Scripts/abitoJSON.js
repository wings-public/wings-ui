...
function bindButtons(file,canvas,base){
    document.getElementById(file).addEventListener('change', function subBind(event) {
      ...
      //  document.getElementById("selectedFile").value = fileName;
        ...
        fileObj.onload = function (e){
            ...
            var painter = new drawTrace(converter.getTraceData(),canvas);
            console.log("tete");
            ...
            var fasta = ">" + header + "\r\n" + basesString; 
            var fastq = "@" + header + "\r\n" + basesString + "\r\n" + "+" + "\r\n" + convertedScores;
            document.getElementById(base).textContent = basesString;
...
function drawTrace (traceData,convas) {
    var colors = {adenine:"green", thymine:"red", guanine:"black", cytosine:"blue", other:"pink"};
    var peakCanvas = document.getElementById(convas);
    ...
    this.drawNumbers = function () {
        ...
        for (var count = 0; count < traceData.baseCalls.length; count++) {
            if (count % 5 === 0) {
                var baseCall = traceData.baseCalls[count];
                ctx.fillStyle = "black";
                ctx.fillText(traceData.basePos[count], traceData.basePos[count] + xOffset, maxHeight - baseBuffer - yOffset);
                console.log("traceData.basePos[count] " + traceData.basePos[count]);
