<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="uc_VariantQuery.ascx.cs" Inherits="WebGAP.GAP.uc.uc_VariantQuery" %>
...
<div class="demo-container gray-bg" id="decorationZone" style="width: 45%; float: left">
    ...
    <div id="leftTabDiv" runat="server" class="disabledDiv pos-rel">
...
<asp:SqlDataSource ID="Ds_FilterSet" runat="server" ConnectionString="<%$ ConnectionStrings:MyDbConn %>" SelectCommand="Sp_GAP_Filter_Set_Sel"
    ...
    <SelectParameters>
        <asp:SessionParameter Name="UserID" SessionField="WiNGSMasterId" Type="Int32" />
                <asp:Parameter Name="AnalysisType" Type="string" DefaultValue="C" />
...
<telerik:RadScriptBlock ID="radSript1" runat="server">
    <script type="text/javascript">
        ...
        function get_reqId(selectedItem) {
            ...
            var VarDisc = '<%= Session["VariantDis_KeyForQuery"] %>' + '<%= Session["VariantDis_ValueForQuery"] %>';
           ...
           // var HostId = 1;
            ...
            $.getJSON(url, function (data) {
                $.each(data, function (HostId, model) {
                    ...
                    document.getElementById("<%= lbl_Status.ClientID  %>").innerText = "in progress...";
                    /*console.log("HostId: " + HostId + " CenterId:" + CenterId + "SelectedLevel: " + SelectedLevel + "FilterId" + FilterId);*/
                    var list = [VarDisc, RefBuild, SeqType, CenterId, HostId, SelectedLevel, FilterId];
                    //console.log("VarDisc: " + VarDisc);
                    ...
                    jQuery.ajax({
                        ...
                        async: true,
                        dataType: 'JSON',
                        url: '/GAP/Service/se_VariantQuery.asmx/GetReqIdAsync',
                        success: function (result) {
                            res = JSON.parse(result.d);

                            var reqId = Number(res["0"].reqId);

                            /*console.log("re: " + reqId)*/
                            counter++;
                            //console.log("CenterId: " + CenterId, "HostId: " + HostId, "FilterId: " + FilterId, "reqID: " + reqId);                        
                            get_resultAsync(CenterId, HostId, reqId, FilterId);
                        },
        ...
        var err = 0; // No error
        function get_resultAsync(centerId, hostId, reqId, FilterId) {
          ...
          /*  console.log(' CenterId:' + CenterId + ' HostId:' + HostId + ' FilterId:' + FilterId + ' ReqId:' + reqId);*/
            ...
            var list = [FilterId, CenterId, HostId, reqId];
            var jsonText = JSON.stringify({ list: list });
            ...
            jQuery.ajax({
                ...
                async: true,
                dataType: 'JSON',
                url: '/GAP/Service/se_VariantQuery.asmx/GetResultAsync',
                beforeSend: function () {
                    var element = document.getElementById("<%= div_loading.ClientID  %>");
                    element.classList.remove("displyNon");
        ...
        var counterSavedFilter = 0;
        function get_reqId_savedFilter(FilterId) {
          
            if (counterSavedFilter === Number('<%= Session["NumberOfCentersAndHosts"] %>')) {
                counterSavedFilter = 0;
           ...
           // var HostId = 1;
            ...
            $.getJSON(url, function (data) {
                $.each(data, function (HostId, model) {
                    ...
                    var list = [VarDisc, RefBuild, SeqType, CenterId, HostId, FilterId];
                    ...
                    jQuery.ajax({
                        ...
                        async: true,
                        dataType: 'JSON',
                        url: '/GAP/Service/se_VariantQuery.asmx/GetReqIdAsync_SavedFilter',
                        success: function (result) {
                            res = JSON.parse(result.d);
                            /*console.log("res: " + result.d)*/
                            var reqId = Number(res["0"].reqId);
                           console.log("re: " + res["0"].reqId)
                            counterSavedFilter++;
                            //console.log("CenterId: " + CenterId, "HostId: " + HostId, "FilterId: " + FilterId, "reqID: " + reqId);                        
                            get_resultAsync_savedFilter(CenterId, HostId, reqId, FilterId);
        ...
        function get_resultAsync_savedFilter(CenterId, HostId, reqId, FilterId) {

            var UserId = '<%= Session["WiNGSMasterId"] %>';
          //  var CenterId = '<%= Session["Current_CenterID"] %>';
         //   var HostId = Number('<%= Session["HostID"] %>');
            ...
            jQuery.ajax({
                ...
                async: true,
                dataType: 'JSON',
                url: '/GAP/Service/se_VariantQuery.asmx/GetResultAsync_SavedFilter',
                beforeSend: function () {
                    var element = document.getElementById("<%= div_loading.ClientID  %>");
                    element.classList.remove("displyNon");
                    document.getElementById("<%= lbl_Status.ClientID  %>").innerText = "Calculation in progress!";
