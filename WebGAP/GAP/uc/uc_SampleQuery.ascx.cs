...
{
    public partial class uc_SampleQuery : System.Web.UI.UserControl
    {
        #region "CnnStr"{
        ...
        */
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            csType = this.GetType();
            csPage = this.Page;
            lbl_sampleID.Text = Session["SampleID_Filter"].ToString() + " | " + Session["FileID_Filter"].ToString() + "(Local Id: " + getLocalID() + ")";
        ...
        protected void AddNewFilter_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(CnnStr))
            {
                using (SqlCommand myCMD = new SqlCommand("Sp_GAP_Add_New_Filter", con))
                {
                    ...
                    myCMD.Parameters["@AnalysisType"].Value = "A";
        ...
        protected void Del_RDD_FilterSet_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(CnnStr))
            {
                using (SqlCommand myCMD = new SqlCommand("Sp_Filter_FilterSetAndItems_Del", con))
                {
                    ...
                    myCMD.Parameters["@tblN"].Value = Session["tblName"].ToString();
                ...
                if (itmIDs != null)
                {
                    ...
                    for (int j = 0; j < itmIDs.Count; j++)
                    {
                        ...
                        if (listOfItm_Ids_Types[1].Equals("RadComboBoxMultiSelected"))
                        {
                            ...
                            RCB.MarkFirstMatch = true;
                            RCB.AllowCustomText = true;
                         //   RCB.KeyboardNavigationSettings = k
        ...
        private void Fetch_Count_Of_Data()
        {
            ...
            SqlCommand cmd = new SqlCommand("Sp_Filter_CountAll_ReqId_Sel", conn);
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add("@FileID", SqlDbType.VarChar, 20);
            ...
            cmd.Parameters["@FileID"].Value = Session["FileID_Filter"].ToString();
            ...
            cmd.Parameters["@CenterID"].Value = Convert.ToInt32(Session["Current_CenterID"].ToString());
            cmd.Parameters["@RefBuild"].Value = Session["RefBuild"].ToString();
            cmd.Parameters["@HostID"].Value = 1;// Convert.ToInt32(Session["HostID"].ToString());
            ...
            if (myDataTable.Rows.Count > 0)
            {
                for (int i = 0; i < myDataTable.Rows.Count; i++)
                {
                    Session["baseCount"] = myDataTable.Rows[i]["result"];
                }
            }
        ...
        private void DisplayData(int selectedLevel)
        {
            ...
            ScriptManager.RegisterStartupScript(this.Page, Page.GetType(), "btnShow", "btnShow_ClientClick(" + selectedLevel.ToString() + ");", true);
        ...
        protected void ShowResultCondition_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("Sp_Filter_Result_Variant_Sel", new SqlConnection(CnnStr));
            cmd.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter SqlDataAdptrCmd = new SqlDataAdapter();
            ...
            cmd.Parameters["@CenterID"].Value = Convert.ToInt32(Session["Current_CenterID"].ToString());
            cmd.Parameters["@RefBuild"].Value = Session["RefBuild"].ToString();
            cmd.Parameters["@HostID"].Value = Convert.ToInt32(Session["HostID"].ToString());
            ...
            try
            {
                ...
                Response.Redirect(String.Format("/GAP/SampleResult.aspx"), false);
