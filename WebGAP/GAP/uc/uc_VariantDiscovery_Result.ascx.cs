...
{
   public partial class uc_VariantDiscovery_Result : System.Web.UI.UserControl
    {
           ...
           // RG_SampleVariant.MasterTableView.CommandItemSettings.ShowExportToExcelButton = false;
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["newQueryIsPassed"].ToString().Equals("true"))
                {
                   ...
                   // string deleteCommand = "Delete FROM Tbl_Filter_Result_VariantDiscovery_Public_Batch_Tracking where TblName= '" + Session["TblName"].ToString() + "' AND [State] = 'done' ";
                    
                        SqlCommand cmd = new SqlCommand("Sp_GAP_VariantDiscovery_Result_Public_Batch_Tracking_Timer_del", new SqlConnection(CnnStr));
                        ...
                        cmd.Parameters["@FilterID"].Value = Session["FilterID_Filter"];
                        cmd.Parameters["@UserID"].Value = Session["WiNGSMasterId"];
        ...
        private void GridBind()
        {
            ...
            SqlCommand Cmd = new SqlCommand("Sp_GAP_VariantDiscovery_Result_Public_Batch_Tracking_Timer_Sel", new SqlConnection(CnnStr));
            Cmd.CommandType = CommandType.StoredProcedure;
        ...
        protected void RG_SampleVariant_ItemCommand(object sender, GridCommandEventArgs e)
        {
           ...
           //     this.RG_SampleVariant.MasterTableView.GetColumn("alt_cnt").Display = true;
                ...
                RG_SampleVariant.ExportSettings.FileName = Session["WiNGSMasterId"].ToString() +"_"+ Session["FilterID_Filter"].ToString();
        ...
        protected void RG_VariantDiscovery_Detail_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                ...
                SqlCommand Cmd = new SqlCommand("Sp_GAP_VariantDiscovery_Public_Result_Detail_GeneAN_Sel", new SqlConnection(CnnStr));
                Cmd.CommandType = CommandType.StoredProcedure;
                SqlDataAdapter SqlDataAdptrCmd = new SqlDataAdapter();
                SqlParameterCollection SqlParCol = default(SqlParameterCollection);
                SqlParCol = Cmd.Parameters;
                SqlParCol.Add(new SqlParameter("@FilterID", SqlDbType.NVarChar));
                SqlParCol.Add(new SqlParameter("@UserID", SqlDbType.VarChar));
                ...
                SqlParCol["@FilterID"].Value = Session["FilterID_Filter"].ToString();
