...
{
    public partial class uc_CSVariant : System.Web.UI.UserControl
    {
        

            bool firstLoadRG = false;
            bool CheckCheckbox_For_Report = false;
            bool Report_HasAlreadyMutation = false;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                Session["Curr_ReportID"] = Session["CarrierScreeningID_Filter"].ToString();
                Session["AnalysisType"] = "1"; //carrier screening
                firstLoadRG = true;
        ...
        protected string Fn_Report_Item_insert(int Report_Version, string CHR, string START, string END, string REF, string ALT, string MutationType, string FilterPath)
        {
            ...
            SqlParCol["@InWhichOneExists"].Value = Session["InWhichOneExists"];
            ...
            SqlParCol["@InputType"].Value = Session["InWhichOneExists"];
        ...
        {
            {
                ...
                if (!String.IsNullOrEmpty(Session["Curr_ReportID"].ToString()) &&
                    ...
                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Myscript1", @"radalert('<br /><b>The session is expired!</b><br />');", true);
