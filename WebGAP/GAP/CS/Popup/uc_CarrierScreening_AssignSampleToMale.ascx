<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="uc_CarrierScreening_AssignSampleToMale.ascx.cs" Inherits="WebGAP.GAP.CS.Popup.uc_CarrierScreening_AssignSampleToMale" %>
...
<telerik:RadAjaxManager ID="RadAjaxManager1" runat="server">
    <AjaxSettings>
        <telerik:AjaxSetting AjaxControlID="RG_SampleForMale">
            <UpdatedControls>
                <telerik:AjaxUpdatedControl ControlID="RG_SampleForMale" LoadingPanelID="RadAjaxLoadingPanel1" />
            </UpdatedControls>
        </telerik:AjaxSetting>
    </AjaxSettings>
...
<div class="content-body pt-1">
    <div class="card-content collapse show">
        <div class="card-bodypopup">
            <form class="form">
                <div class="form-body">
                    <h4 class="form-sectionpopup"><i class="ft-user"></i>Assign Sample To Male</h4>
                    <script type="text/javascript" id="telerikClientEvents1">
                        function RG_Sample_OnColumnMouseOut(sender, args) {
                        }
                    </script>
                    <asp:HiddenField ID="hdn_searchSampleID" runat="server" />
                    <telerik:RadGrid RenderMode="Lightweight" ID="RG_SampleForMale" runat="server" GridLines="Both" OnItemCommand="RG_SampleForMale_ItemCommand" OnItemDataBound="RG_SampleForMale_ItemDataBound"
                        AllowSorting="True" AutoGenerateColumns="False" CellSpacing="-1" Height="350px" DataKeyNames="SampleID"
                        ShowStatusBar="True" Font-Size="X-Small" DataSourceID="DS_SampleForMale">
                        <MasterTableView CommandItemDisplay="Top" DataSourceID="DS_SampleForMale" Name="Master" AutoGenerateColumns="false" ClientDataKeyNames="SampleID" DataKeyNames="SampleID">
                            <CommandItemTemplate>
                                <div>
                                    ...
                                    <telerik:RadComboBox RenderMode="Lightweight" ID="RCB_searchSampleForMale" runat="server" AutoPostBack="True" DataSourceID="DS_searchSampleForMale"
                                        ChangeTextOnKeyBoardNavigation="true" OnClientDropDownOpening="RCB_searchSampleForMaleOpening" OnClientSelectedIndexChanged="RCB_searchSampleForMaleIndexChange"
...
<asp:SqlDataSource ID="DS_SampleForMale" runat="server" ConnectionString="<%$ ConnectionStrings:MyDbConn %>" SelectCommand="Sp_GAP_CarrierScreening_AssignSample_Sel" SelectCommandType="StoredProcedure">
    <SelectParameters>
        ...
        <asp:Parameter Name="IndividualSex" Type="Int32" DefaultValue="1" />
    </SelectParameters>
</asp:SqlDataSource>
<asp:SqlDataSource ID="DS_searchSampleForMale" runat="server" ConnectionString="<%$ ConnectionStrings:MyDbConn %>"
...
<telerik:RadScriptBlock ID="RadScriptBlock1" runat="server">
    <script type="text/javascript">
        ...
        function RCB_searchSampleForMaleIndexChange(sender, eventArgs) {
            var grid = $find("<%= RG_SampleForMale.ClientID %>");
            var radcomboBox = $telerik.findControl(grid.get_element(), "RCB_searchSampleForMale");

            document.getElementById("<%= hdn_searchSampleID.ClientID %>").value = radcomboBox.get_value();
        }

        function RCB_searchSampleForMaleOpening() {
            var grid = $find("<%= RG_SampleForMale.ClientID %>");
            var Com = $telerik.findControl(grid.get_element(), "RCB_searchSampleForMale");
