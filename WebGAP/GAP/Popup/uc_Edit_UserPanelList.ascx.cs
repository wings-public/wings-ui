...
{
    public partial class uc_Edit_UserPanelList : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                int UPanel = 0;
                UPanel = Convert.ToInt32(Session["UPanel_Id"].ToString());

                DataTable data = GetDataTable("exec Sp_GAP_Filter_UPanel_Gene_Sel " + UPanel);

                for (int i = 0; i < data.Rows.Count; i++)
                {
                    RadListBoxItem test_old = new RadListBoxItem(data.Rows[i]["entrez_id"].ToString(), data.Rows[i]["entrez_id"].ToString() + "," + data.Rows[i]["symbol"].ToString());
                    ...
                    lblref.Text = data.Rows[i]["entrez_id"].ToString();
                    lblalias.Text = data.Rows[i]["symbol"].ToString();
                    lbldesc.Text = data.Rows[i]["GDesc"].ToString();
                    RLB_TrainingGenes.Items.Add(test_old);
                }

                DataTable data_upanel_name = GetDataTable("exec Sp_GAP_Filter_UPanel_Name_Sel " + UPanel);
                RTB_UPanelName.Text = data_upanel_name.Rows[0]["UPanelName"].ToString();
            }
        }
        ...
        protected void RC_TrainingGene_Gens_ItemsRequested(object sender, Telerik.Web.UI.RadComboBoxItemsRequestedEventArgs e)
        {
            ...
            for (int i = itemOffset; i <= endOffset - 1; i++)
                RC_TrainingGene_Gens.Items.Add(new RadComboBoxItem(data.Rows[i]["GDesc"].ToString(), data.Rows[i]["entrez_id"].ToString()));
        ...
        protected void RC_TraG_ChrBand_ItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
        {
            ...
            chr = CB_Chromosome.SelectedValue;
        ...
        protected void RB_Submit_Click(object sender, EventArgs e)
        {
            ...
            {
                DeleteAllJobs_and_UpdateUPName();

                //string RPanelId;
                //RPanelId = InsertJob();
                //string PID = RPanelId;

                if (Session["UPanel_Id"].ToString().Length > 0)
                {
                    if (TransferGens(Session["UPanel_Id"].ToString()))
                    {
                        switch (CheckingApi(Session["UPanel_Id"].ToString()))
                        {
                            case 0:
                                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Myscript1", @"alert('Done! Update is on progress!');", true);
                                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "MyFun1", "CloseWindow();", true);
                                break;
                            case 1:
                                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "Myscript1", @"alert('Done! Your panel is already updated!');", true);
                                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "MyFun1", "CloseWindow();", true);
                                break;
                            case -1:
        ...
        public void DeleteAllJobs_and_UpdateUPName()
        {
            string ErrMsg;

            SqlConnection conn = new SqlConnection(CnnStr);
            SqlDataAdapter SqlDataAdptrCmd = new SqlDataAdapter();
            SqlCommand Cmd = new SqlCommand("Sp_GAP_Filter_UPanel_Gene_Del", conn);
            Cmd.CommandType = CommandType.StoredProcedure;
            SqlParameterCollection SqlParCol;
            SqlParCol = Cmd.Parameters;

            SqlParCol.Add(new SqlParameter("@UPID", SqlDbType.Int));
            SqlParCol.Add(new SqlParameter("@UPName", SqlDbType.NVarChar));

            SqlParCol["@UPID"].Value = Session["UPanel_Id"].ToString();
            SqlParCol["@UPName"].Value = RTB_UPanelName.Text;
            ...
            catch (Exception ex)
            {
                ...
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "alertscript", script, true);
            }
        }
        ...
        private int CheckingApi(string pID)
        {
            ...
            SqlConnection conn = new SqlConnection(CnnStr);
            SqlDataAdapter SqlDataAdptrCmd = new SqlDataAdapter();
            SqlCommand Cmd = new SqlCommand("Sp_GAP_Filter_UPanel_Upd_Api", conn);
            Cmd.CommandType = CommandType.StoredProcedure;
            SqlParameterCollection SqlParCol;
            SqlParCol = Cmd.Parameters;
        ...
        public List<ListCVhoosenGene> GetTrainingGeneItems(string RefID)
        {
            ...
            foreach (DataRow row in data.Rows)
            {
                ...
                if(!row[column_Ref].ToString().Equals(null))
        ...
        public List<ListCVhoosenGene> GetTrainingGene_FromChromosomalCoItems()
        {
            ...
            DataTable data = GetDataTable("exec Sp_GAP_Filter_UPanel_ChromosomalCoordinates_Sel '" + type + "','" + Chr + "','" + Start + "','" + Endp + "','" + band + "','" + Session["SelectedRefBuild"] + "'");
        ...
        {
            if (RC_TrainingGen_GOTerms.SelectedValue.Length > 0)
            {
                TrainingGeneColectedItem_Add_Remove();
                List<ListCVhoosenGene> ResList = new List<ListCVhoosenGene>();
